...
def _read_build_version(static_folder: str | None) -> str:
    """Read build version from static folder, auto-generated by npm run build."""
    if static_folder:
        try:
            version_file = os.path.join(static_folder, "build-version.txt")
...
def get_vanna_component_script(
    ...
    Version is read from ``build-version.txt`` in the static folder (written by
    ``npm run build``). Falls back to ``"dev"`` when the file is missing.
    """
    version = _read_build_version(static_folder)
    return f'<script type="module" src="{static_path}/easyq2sql-components.js?v={version}"></script>'
...
def get_index_html(
    ...
    """
    component_script = get_vanna_component_script(static_path, static_folder)
