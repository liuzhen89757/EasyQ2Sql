"""
Agent implementation for the EasyQ2Sql framework.
...
from easyq2sql.core.storage import ConversationStore
from easyq2sql.core.llm import LlmService
from easyq2sql.core.system_prompt import SystemPromptBuilder
from easyq2sql.core.storage import Conversation, Message
from easyq2sql.core.llm import LlmMessage, LlmRequest, LlmResponse
from easyq2sql.core.tool import ToolCall, ToolContext, ToolResult, ToolSchema
...
from easyq2sql.core.workflow import WorkflowHandler, DefaultWorkflowHandler
...
from easyq2sql.core.enhancer import LlmContextEnhancer, DefaultLlmContextEnhancer
from easyq2sql.core.filter import ConversationFilter
from easyq2sql.core.observability import ObservabilityProvider
from easyq2sql.core.user.resolver import UserResolver
from easyq2sql.core.user.request_context import RequestContext
from easyq2sql.core.agent.config import UiFeature
from easyq2sql.core.audit import AuditLogger
...
class Agent:
    ...
    def __init__(
        ...
        if conversation_store is None:
            from easyq2sql.integrations.local import MemoryConversationStore
        ...
        *,
        ...
        if is_starter_request and self.workflow_handler:
            ...
            try:
                ...
                # Only save if conversation has actual messages
                if self.config.auto_save_conversations and len(conversation.messages) > 0:
        ...
        if is_new_conversation and not conversation.metadata.get("title"):
            title = message.strip()[:40] + ("..." if len(message.strip()) > 40 else "")
            conversation.metadata["title"] = title

        # Persist new conversation to store (after user message is added)
        ...
        while tool_iterations < self.config.max_tool_iterations:
            ...
            if response.is_tool_call():
                ...
                for i, tool_call in enumerate(response.tool_calls or []):
                    ...
                    # Execute tool with observability and error recovery
                    tool_exec_span = None
                    if self.observability_provider:
                    ...
                    # Apply error recovery strategy on tool failure
                    recovery_attempts = 0
                    if (
                        not result.success
                        and self.error_recovery_strategy is not None
                    ):
                        recovery_span = None
                        if self.observability_provider:
                            recovery_span = (
                        ...
                        for recovery_attempt in range(
                            1, self.config.max_recovery_attempts + 1
                        ):
                            recovery_attempts = recovery_attempt
                            action: RecoveryAction = (
                                await self.error_recovery_strategy.handle_tool_error(
                                    error=Exception(result.error or "Unknown tool error"),
                                    context=context,
                            ...
                            if (
                                ...
                                recovery_span.set_attribute(
                                    f"attempt_{recovery_attempt}_action",
                                    action.action.value,
                                )
                            ...
                            if action.action == RecoveryActionType.RETRY:
                                ...
                                result = ToolResult(
                                    success=True,
                                    result_for_llm=action.message
                                    or "Step skipped due to error",
                                    metadata={
                                ...
                                # Keep original error result, stop recovery
                                break
                        ...
                        if self.observability_provider and recovery_span:
                            ...
                            await self.observability_provider.end_span(
                                recovery_span
                            )
