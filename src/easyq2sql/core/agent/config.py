...
class AgentConfig(BaseModel):
    ...
    max_recovery_attempts: int = Field(
        default=3, gt=0,
        description="Maximum number of recovery retry attempts per tool execution",
    )
    ...
    auto_extract_schema: bool = Field(
        default=True,
        description="Automatically extract and sync database schema on startup",
    )
