"""Observer for the ``search_table_schema`` tool.

Records execution facts into the shared ``ToolUsageTracker``. Intervention is
done separately by ``easyq2sql.hooks.middleware.schema_regulator.SchemaRegulatorMiddleware``.
"""
...
class SchemaRegulatorHook(LifecycleHook):
    """Observe ``search_table_schema`` and record its execution facts."""
