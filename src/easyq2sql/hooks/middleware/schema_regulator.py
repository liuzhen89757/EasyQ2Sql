"""Intervener for the ``search_table_schema`` tool.

WARN injects guidance into ``system_prompt``; BLOCK additionally removes the
tool from ``request.tools`` (retrieval tool, soft + hard intervention).
"""

import logging

from ..regulator import (
    SCHEMA_SEARCH_POLICY,
    ToolState,
    _request_scope,
    evaluate_state,
...
class SchemaRegulatorMiddleware(LlmMiddleware):
    """Constrain ``search_table_schema``: soft + hard intervention."""

    tool_name = "search_table_schema"
    policy = SCHEMA_SEARCH_POLICY
