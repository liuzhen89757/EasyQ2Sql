...
class NotFoundError(Exception):
    """Fallback NotFoundError for older ChromaDB versions or when chromadb is unavailable."""
...
try:
    ...
    try:
        ...
        NotFoundError = ChromaNotFoundError  # type: ignore[assignment]
    except ImportError:
        pass  # Use the fallback NotFoundError defined above
...
# ---------------------------------------------------------------------------
# Module-level embedding function cache
# ---------------------------------------------------------------------------

_EF_CACHE: Dict[str, object] = {}
...
def _get_or_create_embedding_function(
    device: Optional[str] = None,
):
    """Return a cached embedding function, creating it if not already loaded.

    When *device* is ``None`` (the default), returns ChromaDB's built-in
    ``DefaultEmbeddingFunction`` (ONNX-based, CPU-only).  When *device* is
    set to ``"cuda"``, ``"mps"``, or ``"cpu"``, uses
    ``SentenceTransformerEmbeddingFunction`` which supports GPU inference.

    Model instances are cached at module level so that multiple stores
    (Schema, Metric, Memory) can share a single loaded model.
    """
    if device is None:
        cache_key = "__default_onnx__"
    else:
        cache_key = f"sentence-transformers/all-MiniLM-L6-v2__{device}"
...
class ChromaAgentMemory(AgentMemory):
    ...
    Example:
        >>> from easyq2sql.integrations.chromadb import ChromaAgentMemory
    ...
    def _get_embedding_function(self):
        ...
        If a custom embedding function was provided during initialization it
        is returned as-is.  Otherwise a cached default is used:

        * When ``device`` is ``None`` → ONNX ``DefaultEmbeddingFunction`` (CPU).
        * When ``device`` is set → ``SentenceTransformerEmbeddingFunction``
          with GPU support (CUDA / MPS).
        """
    ...
    async def search_similar_usage(
        ...
        *,
        ...
        def _search():
            ...
            # Hybrid search: vector + keyword → RRF → Cross-Encoder
            ...
            for hr in hybrid_results:
                ...
                metadata_dict = json.loads(meta.get("metadata_json", "{}"))
