...
import './easyq2sql-status-bar';

const meta: Meta = {
  title: 'Components/EasyQ2SqlStatusBar',
  component: 'easyq2sql-status-bar',
  parameters: {
    layout: 'centered',
    backgrounds: {
      default: 'light',
...
export const StatusSequence: Story = {
  render: () => {
    ...
    setTimeout(() => {
      statusBar = document.querySelector('easyq2sql-status-bar');
      updateStatus();
    }, 100);

    return html`
      <div style="width: 500px; padding: 20px;">
        <easyq2sql-status-bar status="idle"></easyq2sql-status-bar>
        <div style="margin-top: 10px; color: #999; font-size: 12px;">
