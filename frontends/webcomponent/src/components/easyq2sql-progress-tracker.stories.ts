...
import './easyq2sql-progress-tracker';

const meta: Meta = {
  title: 'Components/EasyQ2SqlProgressTracker',
  component: 'easyq2sql-progress-tracker',
  parameters: {
    layout: 'centered',
    backgrounds: {
      default: 'light',
...
export const Empty: Story = {
  ...
  render: (args) => html`
    <div style="width: 350px; height: 400px;">
      <easyq2sql-progress-tracker .title=${args.title}></easyq2sql-progress-tracker>
    </div>
  `,
};
...
export const WithTasks: Story = {
  ...
  render: (args) => {
    setTimeout(() => {
      const tracker = document.querySelector('easyq2sql-progress-tracker') as any;
      if (tracker) {
    ...
    return html`
      <div style="width: 350px; height: 400px; background: ${args.theme === 'light' ? '#ffffff' : 'rgb(11, 15, 25)'}; padding: 20px;">
        <easyq2sql-progress-tracker .title=${args.title} theme=${args.theme}></easyq2sql-progress-tracker>
      </div>
    `;
  },
};
...
export const LiveDemo: Story = {
  ...
  render: (args) => {
    ...
    const runDemo = () => {
      if (!tracker) {
        tracker = document.querySelector('easyq2sql-progress-tracker');
        if (!tracker) {
