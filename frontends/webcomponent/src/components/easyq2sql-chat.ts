...
@customElement('easyq2sql-chat')
export class EasyQ2SqlChat extends LitElement {
  static styles = [
    vannaDesignTokens,
    css`
      *, *::before, *::after {
  ...
  @property({ attribute: 'sse-endpoint' }) sseEndpoint = '/api/easyq2sql/v1/chat_sse';
  @property({ attribute: 'ws-endpoint' }) wsEndpoint = '/api/easyq2sql/v1/chat_websocket';
  @property({ attribute: 'poll-endpoint' }) pollEndpoint = '/api/easyq2sql/v1/chat_poll';
  @property() subtitle = '';
  @property() startingState: 'normal' | 'maximized' | 'minimized' = 'normal';
  @property({ attribute: 'conversation-id' }) conversationId: string = '';
  ...
  constructor() {
    ...
    // conversationId is set via @property from attribute or defaults to ''
  }
  ...
  private ensureApiClient() {
    ...
    console.log('[EasyQ2SqlChat] Creating API client with:', {
  ...
  firstUpdated() {
    ...
    // Restore conversation ID from localStorage if not set via attribute
    if (!this.conversationId) {
      try {
        const stored = localStorage.getItem('easyq2sql_last_conversation_id');
        if (stored) this.conversationId = stored;
      } catch { /* ignore */ }
    }

    // Request starter UI or load existing conversation
  ...
  /**
   * Initialize conversation: load history if restoring, or request starter UI.
   * Dispatches 'conversation-ready' when done so parent sidebar can update.
   */
  private async _initConversation(): Promise<void> {
    const storedId = this.conversationId;

    // Check if we have a persisted conversation to restore
    try {
      const lastId = localStorage.getItem('easyq2sql_last_conversation_id');
      if (lastId && lastId === storedId) {
        const detail = await this.getApiClient().getConversation(storedId);
        if (detail && detail.messages && detail.messages.length > 0) {
          ...
          this._dispatchConversationReady();
          return;
        }
      }
    } catch (e) {
      console.warn('[EasyQ2SqlChat] Could not load conversation history:', e);
    }

    // Fallback: request starter UI for new conversation
    await this.requestStarterUI();
    this._dispatchConversationReady();
  }

  /** Notify parent page that conversation is ready (for sidebar refresh). */
  private _dispatchConversationReady(): void {
    try {
      localStorage.setItem('easyq2sql_last_conversation_id', this.conversationId);
    } catch { /* ignore */ }
    this.dispatchEvent(new CustomEvent('conversation-ready', {
      detail: { conversationId: this.conversationId, title: this.title },
  ...
  async loadConversation(conversationId: string): Promise<void> {
    ...
    try {
      localStorage.setItem('easyq2sql_last_conversation_id', conversationId);
    } catch { /* ignore */ }
    this.clearMessages();
    try {
      ...
      console.error('[EasyQ2SqlChat] Error loading conversation:', e);
    }
    this.requestUpdate();
  }

  /** Public API: start a fresh conversation. */
  async startNewConversation(): Promise<void> {
    this.conversationId = this.generateId();
    try {
      localStorage.setItem('easyq2sql_last_conversation_id', this.conversationId);
    } catch { /* ignore */ }
    this.title = 'EasyQ2Sql Chat';
    this.clearMessages();
    await this.requestStarterUI();
    this._dispatchConversationReady();
    this.requestUpdate();
  ...
  /** Public API: delete a conversation by ID. */
  async deleteConversation(conversationId: string): Promise<boolean> {
    try {
      const ok = await this.getApiClient().deleteConversation(conversationId);
      if (ok && conversationId === this.conversationId) {
        await this.startNewConversation();
      }
      return ok;
    } catch (e) {
      console.error('[EasyQ2SqlChat] Error deleting conversation:', e);
      return false;
    }
  }
  ...
  private async _sendMessageInternal(messageText: string): Promise<boolean> {
    ...
    try {
      ...
      // Persist conversation ID + notify parent sidebar
      try {
        localStorage.setItem('easyq2sql_last_conversation_id', this.conversationId);
      } catch { /* ignore */ }
      this.dispatchEvent(new CustomEvent('conversation-ready', {
