...
const addMockEasyQ2SqlChat = (container: HTMLElement) => {
  // Create a mock easyq2sql-chat element with sendMessage method
  const mockEasyQ2SqlChat = document.createElement('div');
  mockEasyQ2SqlChat.setAttribute('id', 'mock-easyq2sql-chat');

  // Store the original querySelector
  const originalQuerySelector = document.querySelector.bind(document);

  // Override querySelector to return our mock when looking for easyq2sql-chat
  document.querySelector = function(selector: string) {
    if (selector === 'easyq2sql-chat') {
      return mockEasyQ2SqlChat as any;
    }
    return originalQuerySelector(selector);
  } as any;

  // Add sendMessage method that logs to console and shows in UI
  (mockEasyQ2SqlChat as any).sendMessage = (message: string) => {
  ...
  container.appendChild(mockEasyQ2SqlChat);
  return mockEasyQ2SqlChat;
};
...
export const SingleButtons: Story = {
  render: () => {
    ...
    addMockEasyQ2SqlChat(container);
