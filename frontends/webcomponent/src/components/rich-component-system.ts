      ...
      actionButtons.forEach((button, index) => {
        ...
        if (action && action.action) {
          ...
          button.addEventListener('click', async () => {
            ...
            // Find easyq2sql-chat component and send message
            const easyq2sqlChat = document.querySelector('easyq2sql-chat') as any;

            if (easyq2sqlChat && typeof easyq2sqlChat.sendMessage === 'function') {
              try {
                const success = await easyq2sqlChat.sendMessage(action.action);
              ...
              console.warn('⚠️ easyq2sql-chat component not found or sendMessage not available');
              button.disabled = false;
              button.classList.remove('button-transitioning', 'button-clicked');
            }
          });
  ...
  render(component: RichComponent): HTMLElement {
    ...
    if (action && !disabled && !loading) {
      button.addEventListener('click', async () => {
        ...
        // Find easyq2sql-chat component and send message with button action
        const easyq2sqlChat = document.querySelector('easyq2sql-chat') as any;
        console.log('   Found easyq2sql-chat:', !!easyq2sqlChat);

        if (easyq2sqlChat && typeof easyq2sqlChat.sendMessage === 'function') {
          console.log('   Calling sendMessage...');
          try {
            const success = await easyq2sqlChat.sendMessage(action);
            if (success) {
          ...
          console.error('   ✗ easyq2sql-chat not found or sendMessage not available');
          // Restore button state if it wasn't originally disabled
          if (!disabled) {
            button.disabled = false;
          }
      ...
      if (buttonConfig.action && !buttonConfig.disabled) {
        button.addEventListener('click', async () => {
          ...
          if (easyq2sqlChat && typeof easyq2sqlChat.sendMessage === 'function') {
            console.log('   Calling sendMessage...');
            try {
              const success = await easyq2sqlChat.sendMessage(buttonConfig.action);
              if (success) {
                console.log('   ✓ Message sent successfully');
              } else {
                console.log('   ✗ Message failed, restoring button state');
...
export class ChartComponentRenderer extends BaseComponentRenderer {
  render(component: RichComponent): HTMLElement {
    ...
    if (plotlyData && Array.isArray(plotlyData) && layout) {
      ...
      if (easyq2sqlChat) {
        chartElement.theme = easyq2sqlChat.getAttribute('theme') || 'dark';
      }
...
export class ArtifactComponentRenderer extends BaseComponentRenderer {
  ...
  private fireArtifactOpenedEvent(component: RichComponent, trigger: 'created' | 'user-action', container: HTMLElement): boolean {
    ...
    // Fire the event from the container element (should bubble up to easyq2sql-chat)
    container.dispatchEvent(event);

    // Also dispatch directly on the easyq2sql-chat element as backup
    const easyq2sqlChat = container.closest('easyq2sql-chat');
    if (easyq2sqlChat) {
      console.log('📡 Also dispatching on easyq2sql-chat element');
      easyq2sqlChat.dispatchEvent(new CustomEvent('artifact-opened', {
        detail: eventDetail,
        bubbles: true,
        cancelable: true
      }));
...
// Tool message component renderer (SQL results, file paths, data)
export class ToolMessageComponentRenderer extends BaseComponentRenderer {
  render(component: RichComponent): HTMLElement {
    const messageEl = document.createElement('easyq2sql-message');
...
export class ComponentRegistry {
  ...
  constructor() {
    ...
    this.register('tool-message', new ToolMessageComponentRenderer());
  }
  ...
  private renderWebComponent(tagName: string, component: RichComponent): HTMLElement {
    ...
    // Set theme to match the parent EasyQ2SqlChat theme
  ...
  private getCurrentTheme(): string {
    // Try to get theme from the parent EasyQ2SqlChat component
    const easyq2sqlChat = document.querySelector('easyq2sql-chat');
    if (easyq2sqlChat) {
      return easyq2sqlChat.getAttribute('theme') || 'dark';
    }
    return 'dark';
  }
...
export class ComponentManager {
  ...
  private triggerScroll(): void {
    // Find the parent easyq2sql-chat component and trigger its scroll method
    const easyq2sqlChat = document.querySelector('easyq2sql-chat') as any;
    if (easyq2sqlChat && typeof easyq2sqlChat.scrollToLastMessage === 'function') {
      // Use requestAnimationFrame to wait for DOM update
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          easyq2sqlChat.scrollToLastMessage();
        });
      });
    }
  }
  ...
  private updateStatusBar(component: RichComponent): void {
    ...
    // Look for easyq2sql-chat and search within its shadow root
    const easyq2sqlChat = document.querySelector('easyq2sql-chat') as any;
    if (easyq2sqlChat && easyq2sqlChat.shadowRoot) {
      statusBar = easyq2sqlChat.shadowRoot.querySelector('easyq2sql-status-bar') as HTMLElement | null;
    }

    // Fallback to document search
    if (!statusBar) {
      statusBar = document.querySelector('easyq2sql-status-bar') as HTMLElement | null;
    }
  ...
  private updateTaskTracker(component: RichComponent): void {
    ...
    const easyq2sqlChat = document.querySelector('easyq2sql-chat') as any;
    if (easyq2sqlChat && easyq2sqlChat.shadowRoot) {
      progressTracker = easyq2sqlChat.shadowRoot.querySelector('easyq2sql-progress-tracker');
    }

    // Fallback to document search
    if (!progressTracker) {
      progressTracker = document.querySelector('easyq2sql-progress-tracker');
    }
  ...
  private updateChatInput(component: RichComponent): void {
    ...
    const easyq2sqlChat = document.querySelector('easyq2sql-chat') as any;
    if (easyq2sqlChat && easyq2sqlChat.shadowRoot) {
      chatInput = easyq2sqlChat.shadowRoot.querySelector('textarea.message-input, input.message-input');
    }
